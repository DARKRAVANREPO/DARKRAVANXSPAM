...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __WBN__")
        end = datetime.now()
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`RUKO ZARA SABAR KARO AATA HU JALDI HI...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __AAJA BABY TU BHI AAJA...__")
        target = ""
        ...
        if str(target) in sudousers:
            ...
            if len(sudousers) > 0:
                ...
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `RUKO ZARA SABAR KARO AATA HU JALDI HI...`")
